...
def main():
    ...
    lensboy_det_dir = project_root / "runs/pipeline1/04_lensboy_full_detection/detections_npz"

    output_dir = project_root / "runs/pipeline1/13_calibcam_all7_from_lensboy_min10_offsets"
    ...
    min_corners = 10
    num_cams = len(config["videos"]["camera_files"])

    print_section("Lensboy -> Calibcam detection converter")
    ...
    for cam_index in range(num_cams):
        npz_path = lensboy_det_dir / f"cam{cam_index}_lensboy_full_detections.npz"
        ...
        for det_i, frame_idx in enumerate(frame_numbers):
            ...
            # Add CCV crop offset to convert cropped MP4 coordinates into full sensor coordinates.
            # cam1-cam4: x0=12, y0=8
            # cam5-cam7: x0=8,  y0=4
            xy_sensor = xy.copy()
            ...
            marker_coords.append(xy_sensor.reshape(-1, 1, 2))

            # We detected every original video frame, so detection index and frame index are the same.
    ...
    print(f"Detection file: {detections_out}")
